#!/bin/sh
# The Å-machine wire parity sweep: the aaterminal sweep's sessions
# driven through both implementations' GlkOte serving loops by the
# same deterministic typist (zglkote_drive.py reading the plain
# input scripts), the update transcripts diffed byte for byte.
#
# Usage and contract as the other wire sweeps: 0 identical, 1
# parted, 2 unusable; VOXAM_REFERENCE names the reference checkout.

set -u

root=$(cd "$(dirname "$0")/.." && pwd)
reference=${VOXAM_REFERENCE:-"$root/../voxam"}
fixtures="$reference/tests/fixtures"

if [ ! -f "$reference/pyproject.toml" ]; then
    echo "certify: no reference implementation at $reference" >&2
    echo "certify: point VOXAM_REFERENCE at the Python voxam checkout" >&2
    exit 2
fi

if ! cargo build --quiet --example aaglkote --manifest-path "$root/Cargo.toml"; then
    echo "certify: the port does not build" >&2
    exit 2
fi

ported="$root/target/debug/examples/aaglkote"

work=$(mktemp -d)
cloak_in="$work/cloak.in"
trap 'rm -rf "$work"' EXIT

# Cloak of Darkness, walked whole: the dark bar, the hook, the
# message, and a polite quit.
cat >"$cloak_in" <<'SCRIPT'
inventory
east
south
read message
north
west
hang cloak on hook
east
south
read message
score
quit
y
SCRIPT

total=0
parted=0

session() {
    name=$1
    story=$2
    script=$3
    total=$((total + 1))

    mkdir -p "$work/ref-$name" "$work/port-$name"

    (cd "$reference" && PYTHONUTF8=1 uv run --quiet python \
        "$root/certify/zglkote_drive.py" "$script" "$work/ref-$name" -- \
        uv run --quiet python "$root/certify/aaglkote_reference.py" \
        "$story" 7) >"$work/$name.ref" 2>&1

    (cd "$reference" && PYTHONUTF8=1 uv run --quiet python \
        "$root/certify/zglkote_drive.py" "$script" "$work/port-$name" -- \
        "$ported" "$story" 7) >"$work/$name.port" 2>&1

    if diff --strip-trailing-cr -q "$work/$name.ref" "$work/$name.port" >/dev/null; then
        echo "IDENTICAL: $name"
    else
        parted=$((parted + 1))
        echo "PARTED: $name"
        diff --strip-trailing-cr "$work/$name.ref" "$work/$name.port" | head -12
    fi
}

session cloak-rel2 "$fixtures/cloak-rel2.aastory" "$cloak_in"
session gosling "$fixtures/gosling.aastory" "$fixtures/gosling.in"
session body_not_status "$fixtures/body_not_status.aastory" "$fixtures/body_not_status.in"
session codepoints "$fixtures/codepoints.aastory" "$fixtures/codepoints.in"

if [ "$parted" -eq 0 ]; then
    echo "certify: $total sessions, every wire telling identical"
    exit 0
fi

echo "certify: $parted of $total sessions parted"
exit 1
