...
| `wasm-diff.sh` | the browser build: every Z acceptance recording driven through the port twice by the same typist -- once over real stdio, once through the wasm module a page loads -- and the transcripts diffed byte for byte, so the browser face ships certified rather than merely looking right in a browser (needs node, the wasm32 target, and a wasm-bindgen CLI matching the crate's version) |
